...
struct FakeNode {
    ...
    void onMessage(const std_msgs::Header::Ptr& /*message*/) const {
...
TEST(UtilsTestingRos, initializationPublisher) {
    ros::NodeHandle nh;
...
TEST(UtilsTestingRos, initializationSubscriber) {
    ros::NodeHandle nh;

    const std::function<void(const std_msgs::HeaderConstPtr&)> callback =
        [](const std_msgs::HeaderConstPtr& /*message*/) {};
    ros::Subscriber sub = nh.subscribe<std_msgs::Header>("/in", 1, callback);
    // should return false - no one here to publish
    EXPECT_FALSE(util_testing::waitForInitalization({sub}, ros::Duration(0.1)));

    FakeNode node;
    EXPECT_TRUE(util_testing::waitForInitalization({sub}));
}
